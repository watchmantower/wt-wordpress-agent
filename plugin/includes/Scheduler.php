...
class Scheduler {
  ...
  public static function on_activate(): void {
    Options::get_instance_id(); // Zorunlu ID üretimi
  ...
  public static function reschedule_after_save($old, $new): void {
    while ($ts = wp_next_scheduled(self::CRON_HOOK)) {
      wp_unschedule_event($ts, self::CRON_HOOK);
    }
  ...
  public static function heartbeat_job(): void {
    $opts = Options::get_all();

    $jitter = rand(0, 30);
    $next   = time() + (int)$opts['interval_sec'] + $jitter;
  ...
  public static function schedule_in(int $sec): void {
    while ($ts = wp_next_scheduled(self::CRON_HOOK)) {
      wp_unschedule_event($ts, self::CRON_HOOK);
    }
